...
# run_name = run{x}, where x is an integer starting from 1.
# first detect if any such runs exist, and set x accordingly.

run_name = "run{}"
i = 1
while os.path.exists(run_name.format(i)):
    i += 1
run_name = run_name.format(i)

log = open(f"{run_name}.log", "w")
...
url = "https://unturned3--yusenh-system-3-modalcontainer-completions.modal.run"
...
async def send_batch_at_scheduled_time(
    ...
    async with semaphore:
        ...
        print(
            f"[t={actual_send_time:.2f}s] Sending batch {batch['batch_id']} "
            ...
            async with (
                session.post(
                    ...
                    timeout=aiohttp.ClientTimeout(
                        total=600
                    ),  # 10 minute timeout # NOTE: you may want to set a manual timeout=600 in your @app.cls() decorator as well
                ) as response
            ):
                ...
                print(
                    f"[t={result['completion_time']:.2f}s] Completed batch {batch['batch_id']} "
...
async def run_batch_simulation(
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        tasks = [
            send_batch_at_scheduled_time(session, batch, url, start_time, semaphore, output_dir)
    ...
    for i, result in enumerate(results):
        if isinstance(result, Exception):
            ...
            processed_results.append({"batch_id": batches[i]["batch_id"], "error": str(result)})
        else:
...
async def main():
    with open("batch_arrivals_pretty.json") as f:
