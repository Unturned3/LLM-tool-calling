...
class ResponseParser:
    ...
    def parse_mmlu(response: str) -> Optional[List[str]]:
        ...
        answer_match = re.search(
            r"The answer is\s*\(?([A-Z])\)?", response, re.IGNORECASE
        )
        if answer_match:
        ...
        try:
            ...
            if json_match:
                ...
                if "paths" in parsed and isinstance(parsed["paths"], list):
                    ...
                    if (
                        parsed["paths"]
                        and isinstance(parsed["paths"][0], dict)
                        and "path" in parsed["paths"][0]
                    ):
...
class GraphEvaluator:
    ...
    @staticmethod
    def evaluate(
        response: str, gold_answer: Dict[str, Any]
...
class InfoBenchEvaluator:
    ...
    def _evaluate_sequential(
        self, request_i: int, meta: Dict[str, Any], predicted_solution: str
    ...
    def evaluate(
        ...
        """
        _, score, details = self._evaluate_sequential(
            request_i, meta, predicted_solution
        )
